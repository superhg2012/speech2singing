...
melblock = MelVocoder(path = "vocoder/melgan-neurips/scripts/logs/DSD")
...
while True:
    ...
    if hp.loss == 'BEGAN':
        ...
        loss_cycle = criterion(speech_2x, fake_speech).mean()
        loss_dis, loss_gan = WGANGP(dis_high, singing[...,:400], fake_singing[...,:400])
        if i % 5 < 2:
            OptimStep([(m, opt, loss_gan + 0.2 * loss_cycle, True),
                (dis_high, opt_dis, loss_dis, False)], 3)
        else:
            OptimStep([(dis_high, opt_dis, loss_dis, False)], 3)
