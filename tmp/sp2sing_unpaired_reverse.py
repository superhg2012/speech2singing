...
while True:
    ...
    fake_speech = F.interpolate(singing, scale_factor=0.5, mode='nearest')
    #mel = (mel-mean)/std
    speech = speech[:,:,:min(fake_speech.size(2), speech.size(2))]
    fake_speech = fake_speech[:,:,:min(fake_speech.size(2), speech.size(2))]
    
    speech = F.pad(speech,(0,(speech.size(2)//64+1)*64 - speech.size(2)), 'reflect')
    fake_speech = F.pad(fake_speech,(0,(fake_speech.size(2)//64+1)*64 - fake_speech.size(2)), 'reflect')
    
    for block in m:
        fake_speech = block(fake_speech)
    
    if hp.loss == 'BEGAN':
        loss_gan, loss_dis, real_dloss, fake_dloss = BEGANLoss(dis_high, speech, fake_speech, k)
    ...
    if (iteration % 50 == 0):
        ...
        idx = random.randint(0, fake_speech.size(0) - 1)

        #mel = (mel * std) +mean
        #z = (z * std) + mean
        real_audio = melblock.inverse(singing).detach().cpu().numpy()
        fake_audio = melblock.inverse(fake_speech).detach().cpu().numpy()
        ...
        logger.log_validation(
            ...
            fake_speech = ("image", plot_spectrogram_to_numpy(), fake_speech[idx]),
            real_speech = ("image", plot_spectrogram_to_numpy(), speech[idx]),

            real_sing_audio = ("audio", 22050, real_audio[idx]),
            fake_speech_audio = ("audio", 22050, fake_audio[idx]),
